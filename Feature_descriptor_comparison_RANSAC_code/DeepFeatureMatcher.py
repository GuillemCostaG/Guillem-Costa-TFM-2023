    ...
    def match(self, img_A, img_B, display_results=0, *args):
        ...
        if self.enable_two_stage:
            ...
            if points_A.size(1) >= 4:
                #He desctivat el RANSAC!!!
                #H_init, _ = cv.findHomography(src, dst, method=0, ransacReprojThreshold=16*np.sqrt(2)+1, maxIters=5000, confidence=0.9999)
                #Per actibar el RANSAC
                H_init, _ = cv.findHomography(src, dst, method=cv.RANSAC, ransacReprojThreshold=16*np.sqrt(2)+1, maxIters=5000, confidence=0.9999)
                (H, status) = cv.findHomography(src,  dst, cv.RANSAC, 4.0)
                print('H DFM')
                print(H_init)
                print('H ransac')
                print(H)
        ...
        if points_A.size(1) >= 4:
            H, status = cv.findHomography(src, dst, method=cv.RANSAC, ransacReprojThreshold=3.0, maxIters=5000, confidence=0.9999)
        ...
        return H, H_init, points_A.numpy(), points_B.numpy(), status
