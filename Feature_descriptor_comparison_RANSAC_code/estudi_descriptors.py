...
def inliers_RANSAC(l_matches, status):
    l_inliners=[]
    for i in range(len(l_matches)):
        if status[i][0]==1 or status[i][0]==True:
            l_inliners.append(l_matches[i])
    return l_inliners
...
def TrasnfArt(imatgeA,transf, nom_imatge, descriptor, fm, environment, opt_CORT=0 ,model_COTR=0):
    ...
    for detector in descriptor:#'LOFTR','COTR','SURF',['ORB','BRISK','SIFT','KAZE', 'AKAZE','DFM']['ORB','DFM']
        ...
        status=rmatch['status']#lolista de llistes amb 0 o 1 o True o False en funcio del matxh si es inliner o no 
        ...
        if n_matches!=0:
            ...
            #inliers amb el RANSAC
            inlinersA=inliers_RANSAC(l_matches_A, status)
            inlinersB=inliers_RANSAC(l_matches_B, status)
        ...
        if len(inlinersA)>3:
            ...
            inlinersA = array_list(inlinersA)#llista aamb les coordenades dels inliners
