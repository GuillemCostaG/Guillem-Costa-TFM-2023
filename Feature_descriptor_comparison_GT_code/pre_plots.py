...
archivo_excel = 'analisi_amb_RANSAC - copia.xlsx'
...
nom_eix_y={'n_kpA':'#kpA', 'n_kpB':'#kpB', 'n_matches':'#matches', 'n_inliers':'#inliers', 'matching_hability':'MA (%)', 'recall':'Recall (%)', 'precision':'Precision (%)', 'repetivitat':'Repeatibility (%)', 'dist_projeccions':'Precision (%)', 'p_random_punts_H':'Percentage of correct points',  'esparcitatA': 'Standard deviation', 'esparcitatB':'Standard deviation', 'temps':'Time (ms)'}
...
for clave1, valor1 in diccionari_resultats_filtrat.items():
    ...
    if primer_string=='esparcitatA':
        ax1.set_title('Spread Original image')  # Añadir título al gráfico
    elif primer_string=='esparcitatB':
        ax1.set_title('Spread Tranformated image')  # Añadir título al gráfico
